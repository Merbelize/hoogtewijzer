...
import { assessScenario, buildFallbackAdvice } from "../public/modules/engine.js";
...
test("standaardtoelichting moedigt hulp vragen bij twijfel aan", () => {
  const explanation = buildFallbackAdvice(
    assessScenario({ fallHeight: "3", avoidHeight: "unknown" })
  );

  assert.match(explanation, /twijfel/);
  assert.match(explanation, /veiliger/);
});
